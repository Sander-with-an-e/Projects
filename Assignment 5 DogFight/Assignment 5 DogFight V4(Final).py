...
while running:
    ...
    # Control red ship (A/D for left/right turns)
    if keys[pygame.K_a]:
        red_angle += omega * dt
    if keys[pygame.K_d]:
        red_angle -= omega * dt

    # Control blue ship (K/; for left/right turns)
    if keys[pygame.K_k]:
        blue_angle += omega * dt
    ...
    if dt > 0.0:
        ...
        # Equations of motion/kinematics for ships
        ...
        red_hit = tmisblue > 0 and distance(red_pos, blue_missile_pos) < 20  # 20 pixels as the collision distance
        blue_hit = tmisred > 0 and distance(blue_pos, red_missile_pos) < 20  # 20 pixels as the collision distance
        ...
        if red_hit or blue_hit:
            ...
            # Print winner and quit the game
            print(f"{winner} wins!")
            pygame.quit()
            sys.exit()
        ...
        screen.blit(red_ship_images[int(red_angle)], (red_pos[0] - red_ship_images[int(red_angle)].get_width() // 2, red_pos[1] - red_ship_images[int(red_angle)].get_height() // 2))
        screen.blit(blue_ship_images[int(blue_angle)], (blue_pos[0] - blue_ship_images[int(blue_angle)].get_width() // 2, blue_pos[1] - blue_ship_images[int(blue_angle)].get_height() // 2))
